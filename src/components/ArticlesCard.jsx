...
const MockData = [
  {
    id: "01",
    title:
      "ADA releases updated infection-control guidance for 2026 in response to evolving aerosol research",
    badge: "ADA",
  ...
  },
  {
    id: "02",
    title:
      "AI-assisted caries detection clears 510(k) hurdle, expected in chairside units by Q4",
    badge: "ID",
  ...
  },
  {
    id: "03",
    title:
      "Survey: 62% of hygienists report burnout symptoms, up six points year-over-year",
    badge: "DHQ",
  ...
  },
  {
    id: "05",
    title:
      "Silver diamine fluoride coverage expands to 24 state Medicaid programs",
    badge: "DiA",
...
const ArticlesCard = () => {
  const { data } = useGetModerationQueueQuery({
    status: "live",
    ordering: "-read_count",
    page_size: 5,
  });
  ...
  const articles = apiResults.length
    ? apiResults.slice(0, 5).map((item, index) => ({
        id: String(index + 1).padStart(2, "0"),
        title: item.ai_title || item.original_article_url,
        badge: item.source_name,
        subtitle: (item.ai_article_categories || []).join(", "),
        price: (item.read_count || 0).toLocaleString(),
      }))
    : MockData;
            ...
            {articles.map((item, index) => (
              <div key={item.id}>
                <div className="flex items-start gap-4 py-3 px-5 hover:bg-gray-100 rounded ">
                  <span className="text-[13px] text-gray-500 font-medium pt-0.5 min-w-[20px]">
                    {item.id}
                  </span>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span className="text-[13px] text-gray-500 font-medium whitespace-nowrap pt-0.5">
                        {item.price}
                      </span>
                    </div>
